...
void loop()
{
    ...
    Serial.println("-- Please enter Base, Arm, Wrist Angles --");
    while (!Serial.available()) {}
    jointAngles[0] = Serial.parseFloat();
    Serial.print("Base: ");
    Serial.println(jointAngles[0]);
    delay(100);
    while (!Serial.available()) {}
    jointAngles[1] = Serial.parseFloat();
    Serial.print("Arm: ");
    Serial.println(jointAngles[1]);
    delay(100);
    while (!Serial.available()) {}
    jointAngles[2] = Serial.parseFloat();
    Serial.print("Wrist: ");
    Serial.println(jointAngles[2]);

    // move to chosen location
    move_toAngles(jointAngles);
